---
title: "Multi-Time Point Analysis of Pupil Data"
author: "Jason Geller"
date: "9/20/2019"
...
Just a few weeks ago [@Mathot] released a great paper on pupillometry giving the reader a basic introduction to the method, along with guidelines on how to preprocess the data, and even offered a few suggestions on how to analyze the data (see the great figure below taken from their paper). This paper will for sure be something I revisit as well as give to folks interested in pupillometry. 


```{r, echo=FALSE, warning=FALSE, Message=FALSE, fig.align='center', fig.width=8, fig.height=12}


library(knitr)

knitr::include_graphics(here::here("pupil_guide.PNG"))
...
Now, let's get on with the blog. Anyone who does eye tracking knows that there are a wide-variety of options when deciding on the appropriate method to use. It is almost impossible to cover every method. When reviewing papers, I feel bad because I inevitable bring up methods the authors did not include--they probably hate me. Well this blog will be another reason for them to hate me. This blog post include another method to the pupillometric analysis toolbox: MTPA. I mostly do these blogs to help me better understand a method, but I also do it in hopes that it will help others. About a two years ago I came across a really great paper by [@yu_multi-time-point_2020] wherein they describe MTPA to analyze fNIRs data. They compared it to the widely used mass univariate analysis. and conclude MTPA is a more power method. Here I show how to apply this method to pupillometry data.


Here we are going to use a simple example from data I collected as a postdoc at the University of Iowa. In this experiment, individuals (*N*=31) heard normal or 6-channel vocoded speech tokens and had to click on the correct picture. The vocoded condition should be harder, and in the figure below you can see the vocoded condition results in larger pupil size than the normal speech condition. If were going to analzye this we might choose a mass univariate approach. Below we fit the time course data using Dale Barr's `clusterperm` package with no corrections (with corrections nothing is significant) and we see that there is a significant difference that emerges starting at 800 ms and ends at around 1900 ms. Using MTPA would we observe something different?
...
pacman::p_load(ERP, mnormt, fdrtool,
               ...
               devtools,randomForest,leaps, pROC, tidyverse, here, gazer, exchangr, clusterperm)
...
timebins1 <- read_csv(here::here("Data", "mtpa_file.csv"))
...
Before jumping into the analysis, I want to explain the method a little bit. The multi-time point analysis uses a Random Forest classifier in conjunction with cross validation to determine if there is a statistical difference in the pupil signal between conditions. This differs slightly from the approach outlined in [@Mathot]. In their approach, linear mixed models are used in conjunction with cross validation to pick samples with the highest Z value. In contrast, MTPA uses all time points. What MTPA does is calculate the area under the curve (AUC) along with 90% confidence intervals based on number of cross validations and averages them. Significance is determined by whether the CIs cross .5. If it does not, there is a difference between condition at that time point. Below I will outline specifics steps to performing MTPA along with the code needed to implement it.

# Running MPTA

1.  We need to read in our sample data (described above) and  `pivot_wide` so time is wide (in columns) and subjects and conditions are long. 

```{r pivot}
### Read Data
# Read the Data preprocessed by gazer 
vocode<- read.csv(here::here("Data", "mtpa_file.csv"))
...
2. In MTPA, we must first partition the data into bandwidths(here two), or the number of points to consider at a time in the analysis—for example with  a bandwidth of 2, a model would be built from two time points (e.g., 1 and 2 and then 2 and 3  and so on and so forth until the last time point has been fit). As a note, [@yu_multi-time-point_2020] recommend 2 and stated other widths did not result in different conclusions. 
...
I have shown how to apply MTPA to pupillometric data. This method appears to be more powerful than a mass univariate approach, showing the whole time course as significant. It would be interesting to see how this compares to the method propose by @mathot.

## Potential Limitations

While this method looks promising, there does appear to be some limitations. First, it look looks like you can't fit multilevel models.If you can, I suspect it is 1) computationally expensive and 2) not be trivial. If anyone has an answer to this I would love to hear it. Second, it appears that you would have to conduct this test for each effect of interest rather then including everything in the model. I will have to do some more investigating, but overall I am quite interested in learning more about ML applications to time course data like pupillometry. 
