...
```{r, echo=FALSE, warning=FALSE, Message=FALSE, fig.align='center', fig.width=4, fig.height=8}
...
2. In MTPA, we must first partition the data into bandwidths(here two), or the number of points to consider at a time in the analysis—for example with  a bandwidth of 2, a model would be built from two time points (e.g., 1 and 2 and then 2 and 3  and so and so forth until the last time point has been fit). As a note, [@yu_multi-time-point_2020] recommend 2 and stated other widths did not result in different conclusions. 
