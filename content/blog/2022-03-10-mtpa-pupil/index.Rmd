...
Now, let's get on with the blog. Anyone who does eye-tracking knows there are a wide-variety of options when deciding on the appropriate method to use. It is almost impossible to cover every method. When reviewing papers, I feel bad because I inevitable bring up methods the authors did not include. This blog post adds to the already complicated landscape of time course analysis. Here we will look at multi-time-point analysis (MTPA). 

I mostly do these blogs to help me better understand a method, but I also do them in hopes that it will help others. About a two years ago I came across a really great paper by [@yu_multi-time-point_2020] wherein they describe MTPA and show how to use it to analyze fNIRs data. I really liked the method and began thinking about all the ways it could be applied (EEG, pupillometry, etc.) In the paper, they showed that it is a more powerful tool than the often used mass univariate approach. I got busy, and did not think about this again until recently. Here I show how to apply this method to pupillometry data.


Here we are going to use a simple example from data I collected as a postdoc at the University of Iowa. In this experiment, individuals (*N*=31) heard normal or 6-channel vocoded speech tokens and had to click on the correct picture. The vocoded condition should be harder, and in the figure below, you can see the vocoded compared to the normal speech condition yields larger pupil size throughout the trial. Commonly, we might test significance by using a mass univariate approach (t-tests at each time point or mixed models). Below we fit the time course data using Dale Barr's `clusterperm` package with no corrections (with corrections nothing is significant) and we see that there is a significant difference that emerges starting at 800 ms and ends at around 1900 ms. Using MTPA would we observe something different?
...
Before jumping into the MTPA analysis, I want to explain the method a little bit. The multi-time- point analysis uses a random forest classifier in conjunction with cross validation to determine if there is a statistical difference in the pupil signal between conditions. This differs slightly from the approach outlined in [@Mathot]. In their approach, linear mixed models are used in conjunction with cross validation to pick samples with the highest Z value. In contrast, MTPA uses all time points. What MTPA does is calculate the area under the curve (AUC) along with confidence intervals based on number of cross validations and averages them. Significance is determined by whether the CIs cross the .5 threshold. If it does not, there is a difference between condition at that time point. Below I will outline specific steps needed to perform MTPA along with the R code. 

# Running MPTA

1.  We need to read in our sample data (described above) and `pivot_wide` so time is wide (in columns) and subjects and conditions are in long format. 
...
2. In MTPA, we must first partition the data into bandwidths(here two), or the number of points to consider at a time in the analysis—-for example with a bandwidth of 2, a model would be built from two time points (e.g., 1 and 2 and then 2 and 3  and so and so forth until the last time point has been fit). See the below matrix as an example. As a note, [@yu_multi-time-point_2020] recommend 2 and stated other widths did not result in different conclusions. 
